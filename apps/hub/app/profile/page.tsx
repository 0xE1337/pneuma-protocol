 ...
 *
 * 三个区段：
 *   1. My Souls：用 useMySouls 反查当前钱包持有的 Souls（"切钱包 = 切身份" 闭环）
 *   2. Browse all Souls：列出全网 mint 过的 tokenId 1..N，作为第三方浏览入口
 *   3. 未连钱包态：只显示第二段，提示连接钱包看 "My Souls"
 *
 * 这是兑现 "wallet-bound identity" 叙事的第一站——用户连钱包后立刻看到属于
 * 自己的 Soul 列表，不再列全网。
 */
        ...
        {!isConnected && (
          <section className="surface-glow p-7 space-y-2">
            <div className="text-[11px] uppercase tracking-[0.18em] text-magenta font-mono">
              Connect to see your Souls
            </div>
            <p className="text-ink-dim text-sm leading-relaxed">
              Wallet-bound identity. Switch wallets in MetaMask and your Soul list updates
              automatically — no login, no session.
            </p>
          </section>
        )}
        ...
        <section className="space-y-4">
          ...
          <div className="surface p-5 space-y-4">
            <p className="text-[12px] text-ink-dim leading-relaxed">
              Anyone can read any Soul's history. Pick a token ID — opens its public
